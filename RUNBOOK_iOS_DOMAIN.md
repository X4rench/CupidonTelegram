...
> а Android/ПК работают — читай этот файл, чини по чек-листу. Обновлено 2026-06-26.

---

## ⚠️ ОБНОВЛЕНИЕ 2026-06-26 — текущее состояние (читать первым)
- **Домен Mini App сейчас: `talkcraft.ru`.** `cupidonapp.ru` снова словил Apple FWW и заброшен для Mini App. nginx-сайт `/etc/nginx/sites-available/talkcraft`, cert `/etc/letsencrypt/live/talkcraft.ru/`. `cupidonai.ru` — по-прежнему оферта/Privacy ЮКассы, НЕ трогать.
- **Имя домена нейтральное намеренно** (без дейтинг-окраса): «cupidon» = романтика = лишний сигнал классификатору. Следующий домен (если talkcraft залетит) — тоже нейтральный + чистый (NXDOMAIN-история).
- **Публичный лендинг — ТРЕЗВЫЙ** (`Landing.tsx` + `<noscript>` в index.html): «обучающий сервис по навыкам общения», юр-данные, Политика/Условия; БЕЗ дейтинга/цен/NSFW. **«Тихий редирект» не-TG трафика на t.me УБРАН** — он сам по себе фрод-сигнал (пустая страница + мгновенный редирект на мессенджер = клоакинг). Фронт теперь домен-агностичен: при переезде хост в коде менять НЕ нужно, только 3 точки входа.
- **Apple FWW ≠ Google.** НЕ трать время на Google Search Console / GSB-review (показывает «чисто», флаг у Apple, не синхронится). Сразу: апелляция **websitereview.apple.com** + переезд на чистый домен.
- **Бот — НА LONG-POLLING, не на webhook.** Stockholm-релей `wh.cupidonai.ru` умер (TCP:443 открыт, HTTPS висит) и недоступен. Перешли на polling: `TG_POLLING=1` + `TG_API_PROXY=` (пусто → исходящие напрямую, Москва→TG работает) в `backend/.env`; код `src/services/bot-poller.js`. Если бот молчит на `/start` — это про polling: проверь `getWebhookInfo` (url должен быть пустой), логи `[poll] getUpdates error`. Если прямой путь к TG заблокируют — вписать рабочий `TG_API_PROXY` или Cloudflare-релей. Поллер — ТОЛЬКО один инстанс.
