...
export function validateRealApproachResult(raw) {
  ...
  const dropNeedyApology = (s) => s
    ...
    .replace(/^\s*(?:(?:о|ой|ну|эх|да|слушай)[,!]?\s+)?(привет[,!]?\s+)?(?:извини(?:те|юсь)?|прости(?:те)?|сорри|сорян|пардон)[^,.!?:-]{0,40}?[,.!?:-]+\s*/i, '$1')
    ...
    .replace(/[\s,–—-]*(?:я\s+)?не\s+(?:маньяк|извращен|псих|больн|сталкер|стрёмн|опасн|куса|подкараул)[а-яё]*(?:\s+как(?:ой|ая|ое|ие)(?:[\s-]+(?:то|нибудь))?)?[\s,–—-]*/gi, ', ')
    // мольба «дай/дашь шанс …» — ставит ниже неё
    .replace(/[,;\s-]*да(?:й|шь)\s+шанс[а-яё]*/gi, ' ')
    ...
    // заходы-просьба разрешения подойти: «не (по)мешаю?», «можно [одну фразу/секунду]?» — нужда
    .replace(/[,;\s-]*не\s+(?:по)?меша[а-яё]*\s*\?/gi, ', ')
