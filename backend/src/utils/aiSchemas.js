...
export function validateRealApproachResult(raw) {
  ...
  const dropNeedyApology = (s) => s
    ...
    // мольба «дай/дашь шанс …» → ведение «давай …» (замена, не удаление — без обрубка)
    .replace(/да(?:й|шь)\s+шанс[а-яё]*/gi, 'давай')
    // просьба разрешения на контакт «можно номер/телегу?» → ведение «давай номер»
    .replace(/можно\s+(?:твой\s+|твою\s+)?(номер|телефон|телеграм|телегу|телегой|телега|контакт)[а-яё]*\s*\?/gi, 'давай $1, ')
    .replace(/,\s*,/g, ',')
    .replace(/([.!?])\s*,/g, '$1')
    .replace(/\s+([,.!?;:])/g, '$1')
    .replace(/^[\s,.;:!?-]+/, '')
