...
function buildGapHint(text, nowTimeStr) {
  try {
    ...
    return `РАЗРЫВ ПО ВРЕМЕНИ: с последнего сообщения прошло ~${days} дн. Диалог остыл → РЕЖИМ ЗАХОДА ЗАНОВО (блок ВРЕМЯ, режим B/C): 9 ответов — это РАЗНЫЕ способы вернуться после паузы (свежий повод/история/новая тема/лёгкая интрига), а НЕ ответы на старые сообщения. НЕ переспрашивай то, что уже спрашивал и что проигнорили. НЕ "ты пропала?/живая?", без оправданий за паузу. Свежий повод — ПРОСТОЙ, ТЁПЛЫЙ и правдоподобный, из реальной жизни (что-то увидел/вспомнил/предложил), БЕЗ сюрреализма и абсурдных выдумок (никаких нелепых историй вроде "кот с паспортом"). Их внутренние шутки — можно лёгким мостиком, но не перегружай и не притягивай за уши.`;
  } catch {
    return '';
  }
}
...
const GOAL_HINTS = {
  ...
  date:         'ЦЕЛЬ С НЕЙ: позвать на свидание — веди к встрече: мостики к оффлайну, при удобном моменте мягкое приглашение-констатация. Если диалог тёплый и затяжной (она вовлечена, играет) — ОДИН из 9 вариантов оседлай текущую тему и мягко позови на конкретную встречу (НЕ каждый раз, НЕ на холодном/раннем диалоге).',
...
const REAL_GOAL_HINTS = {
  ...
  'практика':   'ЦЕЛЬ: практика подходов (побороть страх) — низкие ставки, фокус на чистом подходе, а не результате; спокойнее к отказу, можно лёгкие эксперименты.',
};
  ...
  if (no_analysis) {
    ...
    try {
      const { content, usage, model } = await callAI({
        ...
        messages: [{
          role: 'user',
          content: `<conversation_history>\n${text}\n</conversation_history>\n\nСейчас: ${nowTimeStr}.${gapHint ? `\n\n${gapHint}` : ''}${acqLine}${goalLine}\n\nБЕЗ анализа. Сам определи лучший ход под последнее её сообщение. Верни ТОЛЬКО JSON: {"responses":[{"badge":"дружелюбный|игривый|флирт|уверенный|универсальный","text":"...","why":"коротко по смыслу"}, ... РОВНО 9; среди них 2-3 — вопрос ей, из них 1 - НАСТОЯЩИЙ вопрос узнать ЕЁ (в рамке темы, не сухое интервью)]}.`,
        }],
      });
